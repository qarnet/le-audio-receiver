...
- **UDA1334A (CJMCU-1334 / Adafruit #3678):** Format (SF0/SF1) and MUTE are
  pre-pulled to GND on the Adafruit breakout, so no config wires are needed on that
...
Use whichever DAC you prefer. Pinout below is identical for both DAC choices:
3 wires, no MCK.
...
Use the same D0/D1/D2 (nRF54L15) mapping for BCLK / DIN / LRCK. **Solder-bridge
SCK to GND** on cheap GY-PCM5102 clones (see DAC choice above). No MCK wire.
The legacy E83 mapping is in the section above.
