...
- Warning classification (G1, software/build gate): **zero actionable
  compiler, linker, Kconfig, devicetree, boot, and test warnings** — only the
  documented non-actionable NCS v3.3.0 set (`PARTITION_MANAGER`/sysbuild
  deprecations, `BT_CTLR_CONN_ISO_LOW_LATENCY_POLICY` choice gap,
  experimental `BT_LL_SW_SPLIT`/`BT_CTLR_SET_HOST_FEATURE`/
  `BT_CTLR_PERIPHERAL_ISO`, `__ASSERT()` informational, dongle
  partition-manager deprecations).  This claim covers the G1 gate only; the
  G2 hardware phase produced two OpenOCD flashing diagnostics that are
  classified separately below (not folded into "zero warnings").
...
**nRF54L15/Xiao evidence correction (post-acceptance review):** the first
Xiao capture (`54l15-accepted-receiver.log`) was **REJECTED as acceptance
evidence** — it contains one `<wrn> bt_conn: conn 0x20004fa0 failed to
establish. RF noise?` generated by the rogue bench peer `64:49:7D:E3:53:40`
during the open-pairing window before the controlled central bonded.  A
synchronized rerun on a fresh boot without receiver unpair
(`54l15-v3-accepted-receiver.log`) was performed: the receiver booted
BONDED_ONLY (stale controlled-central bond preserved through flash), a
write-only `bt unpair` opened pairing, and the immediately-launched Mode A
raw helper took the first OPEN advertising opportunity — the controlled
central bonded within 1.8 s and the rogue was excluded by the filter accept
list thereafter.  The **accepted** v3 Xiao capture is warning-free (zero
`<wrn>`, `<err>`, pairing failures, assertions, faults, underruns, stream
resets, decode errors); its single pre-bonding rogue connection attempt
(`Connected: 64:49:7D:E3:53:40` / `reason 0x05`, no warning) is explained by
the deliberate open-pairing window and is not a defect.  E83 evidence
(`e83-accepted-receiver.log`) is unchanged and warning-free.
...
see `54l15-v3-accepted-receiver.log` and `e83-accepted-receiver.log`):

| Stream | Central exit | Receiver counters |
|---|---|---|
| Xiao Mode A (v3) | 0 (3000 frames) | two mono ASEs, gate OPEN, `offload prep OK ... state=ACTIVE`, `I2S DMA started`; Stream[0] SDUs=2834 decoded=6054 plc=386 decode_err=0 i2s_underrun=0 stream_reset=0; Stream[1] SDUs=2851 decode_err=0; clean disconnect + advertising restart |
| Xiao Mode B (v3) | 0 (3000 frames) | single stereo ASE (chan_count=2), gate OPEN, `offload ... state=ACTIVE`, `I2S DMA started`; Stream[0] SDUs=2922 decoded=6054 plc=210 decode_err=0 i2s_underrun=0 stream_reset=0; clean disconnect + advertising restart |
...
faults, zero deadlocks, zero runtime warnings (byte-precise `<wrn>`/`<err>`
scan of both accepted receiver logs), FLPR offload ACTIVE on both Xiao runs
(no unexplained cpuapp fallback), APLL stable on both E83 runs, final
disconnect and advertising restart.  Known 360-frame fallback unchanged;
default 10 ms runs remained FLPR-offloaded.

**G2 flashing diagnostics (classified, not "zero warnings"):** the E83
flash produced two OpenOCD `Warn : Adding extra erase range` diagnostics
(app `0x0005b3e8..0x0005bfff`, net `0x01023d5c..0x01023fff`) — expected
page-tail erase extension before validated `flash write_image`, proven from
build artifacts (image ends `0x0005B3E8`/`0x01023D5C`, 4 KB page ends,
partition ranges `0x00000000..0x000FC000` / `0x01000000..0x01040000`, no
partition crossing).  Recorded disposition: STATUS.md "Build warning
diagnostics" → "OpenOCD flashing diagnostics".  These are documented
expected flashing diagnostics, distinct from the G1 zero-actionable-warning
claim.
...
performed on both receivers, and the accepted Xiao capture uses the
synchronized open-pairing rerun above.  The bench device `64:49:7D:E3:53:40`
repeatedly attempted connections to the Xiao with authentication failures
(pre-existing lab noise, recorded in R0/R1; excluded by the filter accept
list once bonded — zero occurrences in the accepted v3 capture after
bonding).

**Known open follow-up (not silently resolved):** `scripts/bin/fw-flash-dongle`
selects any nRF53 target via `nrf-probes --find nrf53` and feeds that
CMSIS-DAP serial into `interface/jlink.cfg`, failing with `No J-Link device
found` when the J-Link probe is not the resolved one.  Worked around during
revalidation by running the identical OpenOCD sequence with J-Link
auto-detection.  Script fix deferred — see STATUS.md "What does NOT work /
open".
