...
- **T7 accepted** (2026-08-02) — see the T7 acceptance section below; coverage and matrix checks are canonical gate children.
...
### T7 acceptance (2026-08-02)

Closes the coverage-enforcement phase:
...
`bf10c20`, `5ece4d1`, `c6adce8` carry the Stage 2 gap-closing tests and
tooling).

- **First honest numeric baseline committed** — `tests/coverage-baseline.json`
  (schema v1) generated by `test-coverage.sh --write-baseline` on `4a31324`
  from the 23-file manifest numeric population (test-only helper blocks
  GCOVR-excluded): lines **3070/3503 (87.6%)**, branches **1332/1921
  (69.3%)**, functions **182/182 (100.0%)**.  Default enforcement passes on
  the same commit with identical numbers; the ratios are exact post-gap
  measurements and must never be lowered.
...
- Full canonical gate on `4a31324` (detached worktree): all children pass
  including coverage, matrix checker, and BSim; three pristine production
  builds (`fw-build-5340`, `fw-build-54l15`, `fw-build-dongle`) pass with
  zero warnings; `git diff --check` clean; both repos left clean.
