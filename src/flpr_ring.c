 ...
 *
 * Pure SPSC ring implementation. No Zephyr deps beyond stdint/stddef/stdbool.
...
static const uint32_t crc32_table[256] = {
	...
	0x220216B9, 0x5505262F, 0xC5BA3BBE, 0xB2BD0B28, 0x2BB45A92, 0x5CB30A04, 0xC2D7FFA7,
...
void flpr_ring_init(uint8_t *base, enum flpr_ring_dir dir)
{
	...
	memset(base, 0, FLPR_RING_TOTAL_SIZE);

	hdr->magic = FLPR_RING_MAGIC;
	hdr->abi_version = FLPR_RING_ABI_VERSION;
	hdr->flags = 0;
...
bool flpr_ring_validate(const uint8_t *base)
{
	...
	if (hdr->magic != FLPR_RING_MAGIC) {
		return false;
	}
	if (hdr->abi_version != FLPR_RING_ABI_VERSION) {
		return false;
	}
	if (hdr->slot_count != FLPR_RING_SLOT_COUNT) {
		return false;
	}
...
{
	struct flpr_ring_header *hdr = (struct flpr_ring_header *)base;
	uint32_t producer, consumer, space;
