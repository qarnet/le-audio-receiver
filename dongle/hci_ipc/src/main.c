 ...
 *
 * Forked from Zephyr samples/bluetooth/hci_ipc/src/main.c.
 * One change: bt_ctlr_set_public_addr() called before bt_enable_raw()
 * to fix zero-FICR DEVICEADDR on the lab nRF5340DK (see dongle/hci_identity.h).
 */
...
int main(void)
{
	...
	/*
	 * Set the controller's public BD_ADDR before enabling HCI.
	 *
	 * This nRF5340DK's FICR DEVICEADDR is unprogrammed (all zeros),
	 * so without this call the controller advertises 00:00:00:00:00:00
	 * and scanning/connection fails. The address is a lab-only static
	 * identity defined in dongle/hci_identity.h.
	 *
	 * Must be called BEFORE bt_enable_raw(), which internally calls
	 * hci_driver_open() → bt_hci_driver_open() → hci_driver_init() →
	 * sdc_init(). bt_ctlr_set_public_addr() issues a VS HCI command
	 * (SDC_HCI_OPCODE_CMD_VS_ZEPHYR_WRITE_BD_ADDR) via the SDC API
