...
/* Semaphore observability where no public API exists (consume + reset
 * ACK; the stall-ACK semaphore moved to the flpr_acceptance suite). */
...
/* R1 ACK correlation observability (reset side; the stall side lives in
 * the flpr_acceptance module). */
