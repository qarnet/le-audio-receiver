 ...
 *
 * Locks the transactional first-push contract: ten distinct silence
...
ZTEST(audio_i2s, test_startup_480_eleven_blocks_then_start)
{
	test_init_ok();

	zassert_equal(audio_sink_push(test_input_480(), TEST_FRAMES_480 * 2), 0, "startup push");

	zassert_equal(fake_i2s_write_calls(), STARTUP_TOTAL_BLOCKS, "eleven blocks written");
	...
	zassert_equal(fake_i2s_queued_count(), STARTUP_TOTAL_BLOCKS, "all eleven queued");
	...
	/* Eleventh write: exact data (identity) or mock ASRC/offload output. */
...
ZTEST(audio_i2s, test_startup_360_ordering_and_sizes)
{
	...
	zassert_equal(fake_i2s_write_calls(), STARTUP_TOTAL_BLOCKS, "eleven blocks");
	zassert_equal(fake_i2s_trigger_rec(0)->cmd, I2S_TRIGGER_START, "START");
	for (int i = 0; i < STARTUP_SILENCE_BLOCKS; i++) {
...
ZTEST(audio_i2s, test_startup_start_fail_purges_all_eleven)
{
	test_init_ok();
	...
	zassert_equal(fake_i2s_write_calls(), STARTUP_TOTAL_BLOCKS, "all eleven written");
	zassert_equal(fake_i2s_queued_count(), 0, "all eleven DROP-purged");
...
ZTEST(audio_i2s, test_startup_reservoir_survives_ten_block_gap)
{
	test_start_stream();

	/* Emulate ten ordered DMA completions (one 7.5 ms block each):
	 * the eleven-block startup queue must still hold one
...
ZTEST(audio_i2s, test_startup_rateselect_partial_then_482)
{
	...
	zassert_equal(audio_sink_push(test_input_480(), TEST_FRAMES_480 * 2), -ENOSPC,
		      "tenth silence count rejected");

	zassert_equal(fake_i2s_write_calls(), STARTUP_SILENCE_BLOCKS - 1,
		      "nine silence writes before failure");
	zassert_equal(fake_i2s_queued_count(), 0, "earlier silence DROP-purged");
	zassert_equal(fake_i2s_trigger_rec(0)->cmd, I2S_TRIGGER_DROP, "DROP command");
	zassert_equal(test_slab_free(), TEST_SLAB_BLOCKS, "slab fully reclaimable");
	for (int i = 0; i < STARTUP_SILENCE_BLOCKS - 1; i++) {
